...
Customer.getAll = async function (req, callback) {
    ...
    let qryTotal = "select count(*) as total from customers";
    if (_id) {
        var id = "%" + _id + "%"; // Thêm dấu % vào trước và sau id
        qryTotal += " where cus_id LIKE ? or name LIKE ?";
    }
    let data = await query(qryTotal, [id, id]);
    ...
    if (_id) {
        var id = "%" + _id + "%"; // Thêm dấu % vào trước và sau id
        qry += " where cus_id LIKE ? or name LIKE ?";
    }
...
Customer.add_cus = async function (req, callback) {
    let _name = req.body.name;
    let _phoneNumber = req.body.phoneNumber;
    let _email = req.body.email;
    let _address = req.body.address;
    ...
    try {
        ...
        let id = `MHCS${rows}`;
        let qry = "insert into customers values ('" + id + "', '" + _name + "', '" + _email + "','" + _phoneNumber + "','" + _address + "')";
        await query(qry, function(err, data){
